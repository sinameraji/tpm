...
PM v1.2.0 runs against Anthropic (Sonnet 4.6 default, Opus 4.7 deep tier). You pay Anthropic directly at their [published rates](https://www.anthropic.com/pricing). No markup.

A typical fast-tier audit costs **~$1–3** and takes **~8–12 minutes**. Large repos (thousands of files in the map) and deep-tier audits cost more and take longer. These numbers are rough — the pre-flight block in `pm audit` shows the estimate relevant to your tier.
...
- **Historical.** `pm cost` aggregates across audits. `pm report --all` lists audits with their per-audit totals.
...
- **Scope the repo.** Pointing PM at a multi-package monorepo costs more than pointing it at a single app package. PM reads what you give it.
...
- **Drop specific stages to Sonnet when overriding.** `pm config set stage_models.c claude-sonnet-4-6` rolls Stage C back to Sonnet on the deep tier.
